...
@section('content')
    <div class="max-w-2xl">
        <x-ui.card header="Edit Karyawan">
            <form action="{{ route('employees.update', $employee) }}" method="POST">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-ui.select name="gender" :options="['male' => 'Laki-laki', 'female' => 'Perempuan']" :selected="old('gender', $employee->gender)" placeholder="Pilih" />
                    </div>
                    <div>
                        <x-input-label for="birth_date" value="Tanggal Lahir" />
                        <x-text-input id="birth_date" class="block mt-1 w-full" type="date" name="birth_date" :value="old('birth_date', $employee->birth_date?->format('Y-m-d'))" />
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <x-ui.select name="status" :options="['active' => 'Aktif', 'inactive' => 'Nonaktif', 'resigned' => 'Resign']" :selected="old('status', $employee->status)" />
                    </div>
                </div>
                ...
                <div class="grid grid-cols-3 gap-4">
                    <div>
                        ...
                        <x-ui.select name="department_id" :options="$departments->pluck('name', 'id')->toArray()" :selected="old('department_id', $employee->department_id)" placeholder="Pilih" />
                    </div>
                    <div>
                        <x-input-label for="position_id" value="Jabatan" />
                        <x-ui.select name="position_id" :options="$positions->pluck('name', 'id')->toArray()" :selected="old('position_id', $employee->position_id)" placeholder="Pilih" />
                    </div>
                    <div>
                        <x-input-label for="shift_id" value="Shift" />
                        <x-ui.select name="shift_id" :options="$shifts->pluck('name', 'id')->toArray()" :selected="old('shift_id', $employee->shift_id)" placeholder="Pilih" />
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4 mt-4 border-t pt-4">
                    ...
                    <div>
                        ...
                        <x-ui.select name="role" :options="$roles->pluck('name', 'name')->toArray()" :selected="old('role', $employee->user?->roles->first()?->name)" placeholder="Pilih Role" />
                    </div>
                </div>
